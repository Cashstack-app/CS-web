...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-10 left-1/2 transform -translate-x-1/2 z-[1000] flex items-center justify-between px-24 py-6 text-center font-geist w-full rounded-3xl transition-all duration-500 ease-in-out max-w-[950px] h-20  ${
      isScrolled 
      ...
      <div className="flex items-center w-full justify-center">
        ...
        {/* Get Started Button */}
        <div className={`hidden md:block flex-shrink-0 transition-all duration-700 ease-in-out ${
          isScrolled ? 'ml-24' : 'ml-96'
        }`}>
      ...
      {/* Mobile Menu */}
      <div className={`fixed top-[110px] left-[5vw] right-[5vw] backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl transform transition-all duration-300 z-40 ${
        isScrolled 
          ? 'bg-white/95' 
          : 'bg-white/80'
      } ${isMenuOpen ? 'translate-y-0 opacity-100 visible' : '-translate-y-full opacity-0 invisible'}`}>
        <div className="p-8 flex flex-col space-y-6 max-w-6xl mx-auto">
          {navItems.map((item) => (
            <a
              key={item.name}
              href={item.href}
